...
const SearchSection = () => {
  ...
  const regionData = {
    north: {
      name: t('searchSection.regions.north'),
      communities: ["Berber", "Tuareg", "Nubians", "Copts", "Arabs"]
    },
    west: {
      name: t('searchSection.regions.west'),
      communities: ["Yoruba", "Hausa", "Igbo", "Fulani", "Ashanti", "Wolof", "Mandinka"]
    },
    central: {
      name: t('searchSection.regions.central'),
      communities: ["Kongo", "Luba", "Mongo", "Fang", "Bamileke", "Pygmy"]
    },
    east: {
      name: t('searchSection.regions.east'),
      communities: ["Maasai", "Kikuyu", "Swahili", "Amhara", "Oromo", "Somali"]
    },
    southern: {
      name: t('searchSection.regions.southern'),
      communities: ["Zulu", "Xhosa", "San", "Shona", "Tswana", "Sotho", "Ndebele"]
    }
  };
  ...
  return (
    <div className="search-section">
      <h1>{t('searchSection.title')}</h1>

      <div className="search-box">
        <input type="text" placeholder={t('searchSection.searchPlaceholder')} id="search-tribe" />

        <select name="regions" id="regions">
          <option value="All Regions">{t('searchSection.allRegions')}</option>
          <option value="North Africa">{t('searchSection.regions.north')}</option>
          <option value="East Africa">{t('searchSection.regions.east')}</option>
          <option value="West Africa">{t('searchSection.regions.west')}</option>
          <option value="Southern Africa">{t('searchSection.regions.southern')}</option>
        </select>

        <select name="Languages" id="all-languages">
          <option value="All-languages">{t('searchSection.allLanguages')}</option>
          <option value="Gusii">Gusii</option>
          <option value="Yoruba">Yoruba</option>
          <option value="Maa">Maa</option>
          <option value="Gikuyu">Gikuyu</option>
        ...
        <div className="two-buttons">
          <div className="two-buttons-text">
            <p>
              <span className="no-tribes">6</span> {t('searchSection.communitiesFound')}
            </p>
          </div>

          <div className='green-buttons'>
            <button className="hide" onClick={toggleMap}>
              {showMap ? t('searchSection.hideMap') : t('searchSection.showMap')}
            </button>
            <button>{t('searchSection.addCommunity')}</button>
          </div>
        </div>
      </div>
        ...
        <div className="add-map">
          <div className="map-wrapper" ref={mapWrapperRef}>
            ...
            {tooltipData && (
              ...
              >
                <h3 id="tooltipTitle">{tooltipData.name}</h3>
                <p className="communities-label">{t('searchSection.communitiesLabel')}:</p>
                <div className="communities-list" id="tooltipCommunities">
                  {tooltipData.communities.map((community, index) => (
                    <span key={index} className="community-tag">
                      {community}
